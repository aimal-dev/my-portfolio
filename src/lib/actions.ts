...
async function saveDB(data: DB) {
  const { error } = await supabaseAdmin
    .from('settings')
    .upsert({ id: 'main_config', content: data }, { onConflict: 'id' });
...
export async function getHero() {
  const db = await getDB();
  return db.hero || { greeting: "Hi all. I am", name: "Aimal Dev", role: "> Frontend Engineer", githubLink: "https://github.com/example" };
}
